...
class MainActivity : AppCompatActivity(), AsyncBalanceListener {
    ...
    lateinit var alice: KeyPair
    lateinit var bob: KeyPair
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        hdWallet = keyPairService.recoverHdWallet(listOf("acquire", "useful", "napkin", "ranch", "witness", "scare", "lunch", "smart", "sibling", "situate", "potato", "inspire"), null)
        alice = keyPairService.getNextKeyPair(hdWallet)
        bob = keyPairService.getNextKeyPair(hdWallet)
        ...
        GetAccountBalanceTask(this).execute(alice.address, bob.address)
    }
    ...
    private fun sendAE(from: KeyPair, to: KeyPair, amount: BigInteger) {
        var account = aeternityService.accounts.blockingGetAccount(from.address)
        println("Balance: ${UnitConversionUtil.fromAettos(BigDecimal(account.balance), UnitConversionUtil.Unit.AE)}")
        if (amount < account.balance) {
            var spendTx = SpendTransactionModel.builder()
                                .amount(amount)
                                ...
                                .build()
            var result = aeternityService.transactions.blockingPostTransaction(spendTx, from.encodedPrivateKey)
